...
//              "Send Request to Server Timeout: ${error.response.statusCode}";
  ...
  Future<HomeResponse> getHomeList(String sessionId,Map<String, dynamic> data,String lang) async {
    Response response;
    try {
      _dio.interceptors.clear();
      _dio.interceptors
...
//          await writeAuthKey(token);
      ...
      response = await _dio.post(
          "http://api.mypharma-order.com:8080/APIS/api/Home/GetHomePage",data: data);
      return HomeResponse.fromJson(response.data);
    } catch (error, stacktrace) {
      print("Exception occured: $error stackTrace: $stacktrace");
      return HomeResponse.withError(_handleError(error));
    }
  }
